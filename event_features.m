...
LB_one_rise = zeros(1,1);LB_one_fall = zeros(1,1);
LB_two_rise = zeros(1,1);LB_two_fall = zeros(1,1);
LB_three_rise = zeros(1,1);LB_three_fall = zeros(1,1);
LB_five_rise = zeros(1,1);LB_five_fall = zeros(1,1);
LB_seven_rise = zeros(1,1);LB_seven_fall = zeros(1,1);
LB_nine_rise = zeros(1,1);LB_nine_fall = zeros(1,1);
...
if treatment == 'n'
treatment_full = zeros(1,size(Xmax_dvdt,2));
else if treatment == 't'
        treatment_full = ones(1,size(Xmax_dvdt,2));
    end
end
...
while finished == 0
    ...
    if treatment_type == 0    %for original developmental recordings
[finished,file,age,ID] = folder_find('whisk sing', data_base, i);
else if treatment_type ==1      %for trimmed cohort
...
%  
% figure;plot(features(idx==1,1),features(idx==1,2),'r.','MarkerSize',12);hold on;plot(features(idx==2,1),features(idx==2,2),'b.','MarkerSize',12);plot(features(idx==3,1),features(idx==3,2),'g.','MarkerSize',12);plot(C(:,1),C(:,2),C(:,3),'kx');
% 
